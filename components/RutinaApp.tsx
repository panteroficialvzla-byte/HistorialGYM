    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 pb-36 font-sans select-none">
          ...
          <div className="space-y-4">
            {currentExercises.length === 0 ? (
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center">
                <p className="text-xs text-zinc-400 mb-3">No hay ejercicios para este día.</p>
              </div>
            ) : (
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-zinc-900/95 backdrop-blur-lg border-t border-zinc-800 py-2 px-6 z-40">
        <div className="max-w-xl mx-auto flex justify-between items-center">
          <button onClick={() => setActiveTab('today')} className={`flex flex-col items-center gap-1 ${activeTab === 'today' ? 'text-amber-400' : 'text-zinc-500'}`}><Dumbbell className="w-5 h-5" /><span className="text-[10px] font-semibold">Rutina</span></button>
          <button onClick={() => setActiveTab('calendar')} className={`flex flex-col items-center gap-1 ${activeTab === 'calendar' ? 'text-amber-400' : 'text-zinc-500'}`}><CalendarIcon className="w-5 h-5" /><span className="text-[10px] font-semibold">Calendario</span></button>
          <button onClick={() => setActiveTab('charts')} className={`flex flex-col items-center gap-1 ${activeTab === 'charts' ? 'text-amber-400' : 'text-zinc-500'}`}><TrendingUp className="w-5 h-5" /><span className="text-[10px] font-semibold">Progreso</span></button>
          <button onClick={() => setActiveTab('timer')} className={`flex flex-col items-center gap-1 ${activeTab === 'timer' ? 'text-amber-400' : 'text-zinc-500'}`}><Timer className="w-5 h-5" /><span className="text-[10px] font-semibold">Descanso</span></button>
        </div>
      </nav>
    </div>
  );
