...
export default function RequisicionesJMT() {
  ...
  // Notificaciones Inteligentes (Visitas Locales)
  ...
  // Persistir marcas de visto localmente
  useEffect(() => {
  ...
  return (
    <div className={`h-screen flex flex-col md:flex-row font-sans overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      {/* Header Móvil */}
      <div className={`md:hidden px-4 py-3 flex justify-between items-center z-30 border-b ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10">
          ...
          {vistaActiva === 'activos' && (
            <div className="max-w-6xl mx-auto animate-fade-in">
              <div className="mb-6 md:mb-8">
                <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Monitor Operativo</h2>
                <p className={`mt-1 text-sm md:text-base font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Se actualiza automáticamente en tiempo real.</p>
              </div>
              ...
              <div className="space-y-6">
                {pedidosFiltrados.filter(r => r.estado !== 'Recibido').length === 0 ? (
                  ...
                  pedidosFiltrados.filter(r => r.estado !== 'Recibido').map(req => {
                    ...
                    return (
                      <div key={req.id} className={`rounded-2xl border overflow-hidden flex flex-col xl:flex-row transition-colors ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                        <div className={`p-4 md:p-6 flex-1 xl:border-r ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                          <div className="flex justify-between items-start mb-5">
                            <div>
                              <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-2 relative">
                                ...
                                {rolUsuario === 'central' && esNuevo && <span className="absolute -top-1 -right-2 h-2.5 w-2.5 bg-red-500 rounded-full border-2 border-white dark:border-gray-800"></span>}
                                {req.urgencia !== 'Normal' && <span className="bg-red-50 text-red-700 dark:bg-red-900/40 dark:text-red-300 px-2.5 py-1 rounded-md text-xs font-bold tracking-wide">{req.urgencia}</span>}
                              </div>
                              <h3 className="text-lg md:text-xl font-bold">{req.obra}</h3>
                              <p className="text-xs md:text-sm text-gray-500 mt-1">Por {req.solicitante} el {req.fecha}</p>
                            </div>
                          </div>
                          
                          <div className={`rounded-xl p-4 md:p-5 border ${isDarkMode ? 'bg-gray-900/50 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                            ...
                            <ul className="space-y-3">
                              {req.detalles_requisicion?.map((item: any) => (
                                <li key={item.id} className="flex justify-between items-center gap-2">
                                  <span className="font-bold text-sm md:text-base break-words">{item.material}</span>
                                  <div className={`flex items-center gap-1.5 px-3 py-1 rounded-lg border shrink-0 ${isDarkMode ? 'bg-blue-900/30 border-blue-800' : 'bg-blue-50 border-blue-200'}`}>
                                    <span className={`font-black text-sm md:text-lg ${isDarkMode ? 'text-blue-300' : 'text-blue-800'}`}>{formatearNumero(item.cantidad)}</span>
                                    <span className={`font-light text-base md:text-xl ${isDarkMode ? 'text-blue-600' : 'text-blue-300'}`}>/</span>
                                    <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{item.unidad}</span>
                                  </div>
                                </li>
                              ))}
                            </ul>
                        ...
                        <div className={`p-4 md:p-6 xl:w-96 flex flex-col justify-between ${isDarkMode ? 'bg-gray-800/50' : 'bg-gray-50/50'}`}>
                          <div>
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Progreso de la Orden</p>
                            ...
                            {rolUsuario === 'central' ? (
                              <div className="grid grid-cols-2 gap-2 mb-4">
                                {['Solicitado', 'En Facturación', 'En Camino', 'Entregado'].map((est) => (
                                  <button
                                    ...
                                    className={`py-2 px-1 rounded-xl text-[10px] sm:text-xs font-bold border transition-colors ${
                                      req.estado === est
                              ...
                              <div className={`rounded-xl p-3.5 font-bold text-sm text-center mb-6 border ${
                            ...
                            {rolUsuario === 'obra' && req.estado === 'Entregado' && (
                              <div className={`mt-2 p-4 rounded-xl border ${isDarkMode ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'}`}>
                                <p className={`text-xs font-bold mb-3 text-center ${isDarkMode ? 'text-green-400' : 'text-green-800'}`}>Material en sitio (Aviso Central)</p>
                                <button onClick={() => confirmarRecepcionYDescontarStock(req)} className="w-full bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition-colors text-sm">
                          ...
                          <button onClick={() => abrirChat(req)} className={`relative w-full flex items-center justify-center gap-2 mt-4 py-3 border rounded-xl font-bold transition-colors text-sm ${isDarkMode ? 'bg-gray-700 border-gray-600 hover:bg-gray-600 text-gray-200' : 'bg-white border-gray-300 hover:bg-gray-50 text-gray-700'}`}>
          ...
          {vistaActiva === 'historial' && (
            <div className="max-w-6xl mx-auto animate-fade-in">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                <div>
                  <h2 className="text-2xl md:text-3xl font-bold tracking-tight">{rolUsuario === 'central' ? 'Auditoría de Entregas' : 'Archivo Histórico'}</h2>
                  <p className={`mt-1 text-sm md:text-base font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Pedidos confirmados por Obra y descontados del stock.</p>
                </div>
                {rolUsuario === 'central' && (
                  <input type="text" placeholder="Buscar por obra o material..." value={busquedaHistorial} onChange={(e) => setBusquedaHistorial(e.target.value)} className={`w-full md:w-72 font-medium px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`} />
                )}
              </div>
              ...
              {rolUsuario === 'central' ? (
                      ...
                      <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-100'}`}>
                        {pedidosFiltrados.filter(r => r.estado === 'Recibido' && (r.obra.toLowerCase().includes(busquedaHistorial.toLowerCase()) || r.detalles_requisicion?.some((i:any) => i.material.toLowerCase().includes(busquedaHistorial.toLowerCase())))).map(req => (
                          <tr key={req.id} className={`transition-colors ${isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'}`}>
                            <td className="p-4 md:p-5 font-bold text-gray-500 text-xs">REQ-{req.id}</td>
                            <td className="p-4 md:p-5 font-bold">{req.obra}</td>
                            ...
                            <td className="p-4 md:p-5 text-xs md:text-sm text-gray-500 font-medium">{req.fecha}</td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5">
                  {pedidosFiltrados.filter(r => r.estado === 'Recibido').map(req => (
                    <div key={req.id} className={`rounded-2xl border p-5 md:p-6 relative overflow-hidden transition-colors ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                      ...
                      <button onClick={() => abrirChat(req)} className="mt-4 text-[10px] md:text-sm font-bold text-[#011C39] dark:text-blue-400 hover:underline flex items-center gap-1"><IconHistory/> Ver bitácora completa</button>
                    </div>
                  ))}
                  {pedidosFiltrados.filter(r => r.estado === 'Recibido').length === 0 && (
          ...
          {vistaActiva === 'nuevo' && rolUsuario === 'obra' && (
            <div className="max-w-4xl mx-auto animate-fade-in">
              <div className="mb-6 md:mb-8">
                <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Solicitud de Materiales</h2>
                <p className={`mt-1 text-sm md:text-base font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Construya su requisición a partir del catálogo central.</p>
              </div>
              
              <div className={`rounded-3xl border p-4 md:p-8 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <h3 className="font-bold mb-4 text-base md:text-lg">1. Seleccionar Insumos</h3>
                <div className="flex flex-col md:flex-row gap-3 mb-8">
                  <select value={itemSeleccionado} onChange={(e) => setItemSeleccionado(e.target.value)} className={`flex-1 font-bold p-3.5 md:p-4 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`}>
                  ...
                  <div className="flex gap-2 md:gap-3">
                    <input type="number" placeholder="Cant." value={cantidadItem} onChange={(e) => setCantidadItem(e.target.value)} className={`w-full sm:w-28 font-bold p-3.5 md:p-4 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} min="0.1" step="0.1" />
                ...
                <h3 className="font-bold mb-4 text-base md:text-lg">2. Resumen del Pedido</h3>
                    ...
                    <ul className="space-y-2">
                      {itemsPedido.map((item, i) => (
                        <li key={i} className={`flex justify-between items-center p-3 md:p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200'}`}>
                          <span className="font-bold text-sm md:text-base truncate pr-2">{item.material}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 mb-8">
                  <div>
                    <label className="block text-xs md:text-sm font-bold mb-2">Nivel de Prioridad</label>
                    <select value={nuevoPedido.urgencia} onChange={(e) => setNuevoPedido({...nuevoPedido, urgencia: e.target.value})} className={`w-full font-bold p-3.5 md:p-4 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`}>
                  ...
                  <div>
                    <label className="block text-xs md:text-sm font-bold mb-2">Notas / Justificación</label>
                    <input type="text" value={nuevoPedido.notas} onChange={(e) => setNuevoPedido({...nuevoPedido, notas: e.target.value})} className={`w-full font-bold p-3.5 md:p-4 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} placeholder="Ej: Material para vaciado del viernes..." />
                  </div>
                </div>

                <button onClick={procesarNuevoPedido} disabled={cargando || itemsPedido.length === 0} className="w-full bg-[#011C39] text-white rounded-xl py-3.5 md:py-4 font-bold text-base md:text-lg hover:bg-blue-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {vistaActiva === 'catalogo' && rolUsuario === 'central' && (
            <div className="max-w-6xl mx-auto animate-fade-in">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
                <div>
                  <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Inventario Maestro</h2>
                  <p className={`mt-1 text-sm md:text-base font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Gestión del catálogo y control de stock central.</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                  <input type="text" placeholder="Buscar material..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className={`w-full sm:w-auto flex-1 md:w-64 border font-medium px-4 py-3 rounded-xl outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900'}`} />
                  <button onClick={() => abrirModalCatalogo()} className="bg-[#011C39] text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-900 whitespace-nowrap text-sm md:text-base">
                    + Añadir Insumo
                  </button>
                </div>
              </div>
              ...
              <div className={`rounded-2xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className="overflow-x-auto">
                  <table className="w-full text-left whitespace-nowrap">
                    <thead className={`text-xs md:text-sm ${isDarkMode ? 'bg-gray-900 text-gray-300 border-gray-700' : 'bg-gray-50 text-gray-600 border-gray-200'} border-b`}>
                      <tr>
                    ...
                    <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-100'}`}>
                      {catalogoOrdenado.filter(c => c.material.toLowerCase().includes(busqueda.toLowerCase())).map(item => (
                        <tr key={item.id} className={`transition-colors ${isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'}`}>
                          <td className="p-4 md:p-5 text-gray-500 font-mono text-[10px] md:text-xs font-bold">{item.codigo_sku || 'N/A'}</td>
                          <td className="p-4 md:p-5 font-bold text-sm md:text-base">{item.material}</td>
                          <td className="p-4 md:p-5">
                            <span className={`px-2 md:px-3 py-1 rounded-md text-[10px] md:text-xs font-bold ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>{item.categoria}</span>
                          </td>
                          <td className="p-4 md:p-5 text-gray-500 font-bold text-xs md:text-sm">{item.unidad}</td>
                          ...
                          <td className="p-4 md:p-5 text-right">
                            <div className="flex justify-end gap-1 md:gap-2">
                              <button onClick={() => abrirModalCatalogo(item)} className="p-1 md:p-2 text-blue-500 hover:bg-blue-100 dark:hover:bg-gray-700 rounded-lg"><IconEdit /></button>
                              <button onClick={() => eliminarCatalogo(item.id)} className="p-1 md:p-2 text-red-500 hover:bg-red-100 dark:hover:bg-gray-700 rounded-lg"><IconTrash /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {vistaActiva === 'usuarios' && rolUsuario === 'central' && (
            <div className="max-w-6xl mx-auto animate-fade-in">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
                <div>
                  <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Cuentas y Accesos</h2>
                  <p className={`mt-1 text-sm md:text-base font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Control de usuarios del sistema.</p>
                </div>
                <button onClick={() => abrirModalUsuario()} className="w-full sm:w-auto bg-[#011C39] text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-900 text-sm md:text-base">
                  + Registrar Acceso
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {usuarios.map(user => (
                  <div key={user.id} className={`rounded-2xl p-5 md:p-6 border relative group ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                    <div className="flex justify-between items-start mb-4">
                    ...
                    <h3 className="font-bold text-base md:text-lg">{user.nombre}</h3>
                    <p className={`text-xs md:text-sm mb-4 font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{user.email}</p>
                    <div className={`flex items-center gap-2 mt-4 pt-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                      <span className={`px-2.5 py-1 rounded-md text-[9px] md:text-[10px] font-bold uppercase tracking-wider ${user.rol === 'central' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'}`}>{user.rol}</span>
                      <span className="text-xs md:text-sm font-bold truncate">{user.obra_asignada}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {modalChat && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 z-50">
          <div className={`rounded-3xl w-full max-w-lg h-[85vh] sm:h-[80vh] flex flex-col overflow-hidden animate-fade-in-up border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className={`p-4 md:p-6 border-b flex justify-between items-center z-10 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
              <div>
                <h3 className="font-bold text-lg md:text-xl">Requisición REQ-{modalChat.id}</h3>
                <p className={`text-xs md:text-sm font-bold mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{modalChat.obra}</p>
              </div>
            ...
            <div className={`flex-1 p-4 md:p-6 overflow-y-auto flex flex-col space-y-4 md:space-y-5 ${isDarkMode ? 'bg-gray-900/50' : 'bg-gray-50'}`}>
              {mensajesChat.length === 0 ? (
                <div className="m-auto text-center">
                  ...
                  <p className="font-bold text-gray-500 text-sm">Aún no hay comentarios.</p>
                </div>
              ) : (
                mensajesChat.map((msg, idx) => {
                  ...
                  return (
                    <div key={idx} className={`flex flex-col ${esMio ? 'items-end' : 'items-start'}`}>
                      <span className="text-[9px] md:text-[10px] font-bold text-gray-500 mb-1 ml-1">{msg.emisor}</span>
                      <div className={`max-w-[85%] p-3 md:p-3.5 rounded-2xl text-xs md:text-sm font-medium ${esMio ? 'bg-[#011C39] text-white rounded-tr-none' : isDarkMode ? 'bg-gray-700 text-white rounded-tl-none border border-gray-600' : 'bg-white text-gray-900 border border-gray-200 rounded-tl-none'}`}>
                        {msg.mensaje}
                      </div>
                    </div>
                  );
            ...
            <form onSubmit={enviarMensaje} className={`p-3 md:p-5 border-t flex gap-2 md:gap-3 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
              <input type="text" value={nuevoMensaje} onChange={(e) => setNuevoMensaje(e.target.value)} placeholder="Escribe un mensaje..." className={`flex-1 border rounded-xl px-3 md:px-4 py-2.5 md:py-3 font-medium outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300 text-gray-900'}`} required />
              <button type="submit" className="bg-[#011C39] text-white h-10 w-10 md:h-12 md:w-12 shrink-0 rounded-xl flex items-center justify-center hover:bg-blue-900 transition-colors"><IconSend /></button>
            </form>
          </div>
        </div>
      )}
      ...
      {modalUsuario && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 z-50">
          <div className={`rounded-3xl w-full max-w-md overflow-hidden animate-fade-in-up border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className={`p-4 md:p-6 border-b flex justify-between items-center ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
              <h3 className="font-bold text-lg md:text-xl">{usuarioEditando ? 'Editar Usuario' : 'Nuevo Registro'}</h3>
            ...
            <form onSubmit={guardarUsuario} className="p-6 md:p-8 space-y-4 md:space-y-5">
              <div>
                <label className="block text-xs md:text-sm font-bold mb-2">Nombre Completo</label>
                <input type="text" value={formUsuario.nombre} onChange={(e) => setFormUsuario({...formUsuario, nombre: e.target.value})} className={`w-full font-medium px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} required />
              </div>
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-bold mb-2">Rol</label>
                  <select value={formUsuario.rol} onChange={(e) => setFormUsuario({...formUsuario, rol: e.target.value})} className={`w-full font-bold px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`}>
                    <option value="obra">Obra</option><option value="central">Central</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-bold mb-2">Asignación</label>
                  <input type="text" value={formUsuario.obra_asignada} onChange={(e) => setFormUsuario({...formUsuario, obra_asignada: e.target.value})} className={`w-full font-medium px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} placeholder="Ej: Torre A" />
                </div>
              </div>
              <div>
                <label className="block text-xs md:text-sm font-bold mb-2">Correo de Acceso</label>
                <input type="email" value={formUsuario.email} onChange={(e) => setFormUsuario({...formUsuario, email: e.target.value})} className={`w-full font-medium px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} required disabled={!!usuarioEditando} />
              </div>
              <div>
                <label className="block text-xs md:text-sm font-bold mb-2">Contraseña</label>
                <input type="text" value={formUsuario.password} onChange={(e) => setFormUsuario({...formUsuario, password: e.target.value})} className={`w-full font-medium px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} required />
              </div>
              <button type="submit" disabled={cargando} className="w-full bg-[#011C39] text-white rounded-xl py-3.5 md:py-4 font-bold hover:bg-blue-900 transition-colors mt-4 md:mt-6 text-sm md:text-base">
      ...
      {modalCatalogo && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 z-50">
          <div className={`rounded-3xl w-full max-w-md overflow-hidden animate-fade-in-up border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className={`p-4 md:p-6 border-b flex justify-between items-center ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
              <h3 className="font-bold text-lg md:text-xl">{catalogoEditando ? 'Editar Insumo' : 'Nuevo Insumo'}</h3>
            ...
            <form onSubmit={guardarCatalogo} className="p-6 md:p-8 space-y-4 md:space-y-5">
              <div>
                <label className="block text-xs md:text-sm font-bold mb-2">Nombre del Material</label>
                <input type="text" value={formCatalogo.material} onChange={(e) => setFormCatalogo({...formCatalogo, material: e.target.value})} className={`w-full font-bold px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} required />
              </div>
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-bold mb-2">Categoría</label>
                  <select value={formCatalogo.categoria} onChange={(e) => setFormCatalogo({...formCatalogo, categoria: e.target.value})} className={`w-full font-bold px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`}>
                ...
                <div>
                  <label className="block text-xs md:text-sm font-bold mb-2">Unidad</label>
                  <select value={formCatalogo.unidad} onChange={(e) => setFormCatalogo({...formCatalogo, unidad: e.target.value})} className={`w-full font-bold px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`}>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-bold mb-2">SKU (Opcional)</label>
                  <input type="text" value={formCatalogo.codigo_sku} onChange={(e) => setFormCatalogo({...formCatalogo, codigo_sku: e.target.value})} className={`w-full font-bold px-4 py-3 rounded-xl border outline-none focus:border-[#011C39] transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-bold mb-2 text-green-600 dark:text-green-400">Stock Inicial</label>
                  <input type="number" value={formCatalogo.stock} onChange={(e) => setFormCatalogo({...formCatalogo, stock: Number(e.target.value)})} className={`w-full font-black px-4 py-3 rounded-xl border outline-none focus:border-green-600 transition-all text-sm md:text-base ${isDarkMode ? 'bg-gray-900 border-gray-600 text-white' : 'bg-gray-50 border-gray-300'}`} min="0" step="0.1" />
                </div>
              </div>
              <button type="submit" disabled={cargando} className="w-full bg-[#011C39] text-white rounded-xl py-3.5 md:py-4 font-bold hover:bg-blue-900 transition-colors mt-4 md:mt-6 text-sm md:text-base">
